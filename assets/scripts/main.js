$(document).ready(function () {
  ...
  $(document).mouseup(function (e) {
    if (
      !searchFilterContainer.is(e.target) &&
      searchFilterContainer.has(e.target).length === 0
    ) {
      $("body").removeClass("filter-search-open");
    }
  });
  searchFilterSale
    .add(searchFilterRent)
    .add(searchFilterOverlay)
    .click(function () {
      $("body").removeClass("filter-search-open");
    });
    ...
    $(".tab").on("click", function (evt) {
      ...
      if ($this.hasClass("active")) {
        ...
        $this
          ...
          .removeClass("active");
        $(
          '.tab-holder .tab-content[data-tab="' +
            $theTab +
            '"], .tab[id="' +
            $theTab +
            '"]'
  ...
  $(".card-holder__carousel").each(function () {
    ...
    slickInduvidual.slick({
      ...
      dots: false,
      arrows: true,
      prevArrow: slickInduvidual
        .parent()
        .parent()
        .siblings()
        .find(".card-navigation__prev"),
      nextArrow: slickInduvidual
        .parent()
        .parent()
        .siblings()
        .find(".card-navigation__next"),
      draggable: false,
      autoplay: false,
      speed: 600,
    });
